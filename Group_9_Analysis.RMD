...
analysis.country <- analysis %>%
  mutate(Qualityclassindicator = as.numeric(Qualityclass=="Good")) %>%
  group_by(country_of_origin) %>%
  summarise(good_quality_porprotion = round(mean(Qualityclassindicator), 2),
            number_of_batch = n())
...
The following table filter countries and its number of batch with 20% good quality rate before and after, which proviedes more detailed information than the above boxplot.
```{r, eval = TRUE}
...
analysis.country %>%
  ...
  kable(caption = '\\label{tab:countryodd} Origins with 20% good quality rate before and after', booktabs = TRUE,
...
```{r , echo=FALSE, fig.width = 13, fig.align = "center", fig.cap = "Boxplots of countinous features on different quality class.", fig.pos = 'H', message = FALSE}
...
``` {r category_two_defects4, echo=FALSE, fig.width = 13, fig.align = "center", fig.cap = "Histogram for category two defects and its log transformation.", fig.pos = 'H', message = FALSE}
...
The following two hists shows the effect of remving outliers on mean altitude
...
Here is 6 boxplots comparing features distribution between good and poor coffee.
...
#Determine the number of observations by year.
...
```{r}
# Step AIC on the full model directly has warnings.

stepAIC(model_all_4, direction = 'both')
...
#The same model was selected using both approaches. Fit this final model to the full data set since neither year nor altitude level are included (the variables with missing values).
...
```{r}
coffee_final$prid <- predict(model_best, coffee_final, type='response')
